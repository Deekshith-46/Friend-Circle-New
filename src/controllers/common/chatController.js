...
exports.startChat = async (req, res) => {
  try {
    ...
    const maleId = req.user.id; // Male user is the authenticated user initiating the chat
    const senderType = req.user.type;
    
    // Check chat permissions
    const permission = await checkChatPermission(
      req.user.id, // male user id
      req.user.type, // 'male'
      femaleId, // receiver
      'female'
    );
    ...
    const userIds = [femaleId.toString(), maleId.toString()].sort();
...
exports.getChatRooms = async (req, res) => {
  try {
    const userId = req.user.id;
    const userType = req.user.type; // Assuming this is set in auth middleware
    ...
    const roomsWithUnread = await Promise.all(populatedRooms.map(async (room) => {
      ...
      // Find the last read time for this user in this room
      const userLastRead = roomObj.lastReadBy?.find(entry => 
      ...
      const unreadCount = await Message.countDocuments({
        ...
        createdAt: { $gt: userLastRead?.lastReadAt || new Date(0) },
